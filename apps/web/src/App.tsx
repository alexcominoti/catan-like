...
export function App() {
  ...
  // Modais: seletor de recurso (cartas) e construtor de troca.
    ...
    <div className="app">
      <header className="header">
        ...
        <div className="phase">
          ...
          <strong style={{ color: PLAYER_FILL[state.currentPlayer] }}>{PLAYER_LABEL[state.currentPlayer]}</strong>
          {state.pendingFreeRoads > 0 && <span className="badge"> {state.pendingFreeRoads} estrada(s) grátis</span>}
        </div>
        <button onClick={() => { setState(newGame()); setLog(['Nova partida.']); setMode('idle'); setError(null); resetTransient(); }}>
          Novo jogo
        </button>
      </header>
        ...
        <div className="card">
          ...
          {state.players.map((p) => (
            <div key={p.color} className={`player-row${p.color === state.currentPlayer ? ' active' : ''}`}>
              ...
              {state.longestRoad.owner === p.color && <span className="badge">📏</span>}
              {state.largestArmy.owner === p.color && <span className="badge">⚔️</span>}
          ...
          {cur.progressCards.length === 0 ? (
            ...
            <div className="dev-cards">
              {PLAYABLE_CARDS.map(({ card, label }) =>
                devCounts[card] ? (
                  <button key={card} disabled={!canPlay(card)} onClick={() => playCard(card)}>
                    {label} ×{devCounts[card]}
                  </button>
                ) : null,
              )}
              {devCounts.victoryPoint ? (
                <span className="res-chip">⭐ Ponto ×{devCounts.victoryPoint}</span>
              ) : null}
            </div>
          )}
        </div>
      ...
      <div className="actionbar">
        {state.phase === 'ended' ? (
          ...
          <>
            ...
            <button className={modeBtn(mode, 'buildRoad')} disabled={!isMain && state.pendingFreeRoads === 0} onClick={() => toggle('buildRoad')}>Estrada</button>
            <button className={modeBtn(mode, 'buildSettlement')} disabled={!isMain} onClick={() => toggle('buildSettlement')}>Vila</button>
            <button className={modeBtn(mode, 'buildCity')} disabled={!isMain} onClick={() => toggle('buildCity')}>Cidade</button>
            <button disabled={!isMain} onClick={() => dispatch({ t: 'buyProgressCard' })}>Comprar carta</button>
            <span style={{ display: 'inline-flex', gap: 4, alignItems: 'center' }}>
              ...
              <button disabled={!isMain} onClick={() => dispatch({ t: 'tradeBank', give, want })}>{bestRate}:1</button>
            </span>
      ...
      {arming === 'monopoly' && (
        <ResourcePickerModal
          title="Monopólio — escolha o recurso"
          onPick={(r) => dispatch({ t: 'playMonopoly', resource: r })}
          onClose={() => setArming(null)}
        />
      )}
      {arming === 'yearOfPlenty' && (
        <ResourcePickerModal
          title={`+2 Recursos — escolha ${2 - yopPicks.length} (${yopPicks.map((r) => RESOURCE_ICON[r]).join(' ')})`}
          onPick={(r) => {
      ...
      {state.activeTrade && (
        <ActiveTradeModal state={state} dispatch={dispatch} />
      )}
    </div>
  );
  ...
  return (
    <div className="overlay" onClick={onClose}>
      <div className="modal" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="hand">
          {RESOURCES.map((r) => (
            <button key={r} onClick={() => onPick(r)}>{RESOURCE_ICON[r]} {RESOURCE_LABEL[r]}</button>
          ))}
        </div>
    ...
    <div className="overlay" onClick={onClose}>
      <div className="modal wide" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="modal-actions">
          <button
            className="primary"
            disabled={(total(tradeGive) === 0 && total(tradeWant) === 0) || to.length === 0}
            onClick={() => onPropose(to)}
          >
            Propor
          </button>
    ...
    <div className="overlay">
      <div className="modal" onClick={(e) => e.stopPropagation()}>
        ...
        <p className="trade-summary">
          Dá <b>{fmt(t.give)}</b> &nbsp;→&nbsp; quer <b>{fmt(t.want)}</b>
        </p>
...
function DiscardControls({
  ...
  return (
    <>
      <span className="hint">
        Rolou 7! Descarte pela metade: {pending.map(([c, n]) => `${PLAYER_LABEL[c]} (${n})`).join(', ')}
      </span>
